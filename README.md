...
One pooled multimodal J-lens is fitted on balanced text, image and
spoken-caption examples. The model's `bird` and `cat` coordinates are then
exchanged with an exact alpha=1 intervention across the contiguous L16-L40
band, at every original prompt position. The answer is generated freely: no
answer token is appended, no candidate list selects the result, and nothing is
teacher-forced.

On a fresh, frozen population of 16 photographs, that exchange changed the
downstream **leg-count** answer from two to four in 14 of 16 text trials, 16 of
16 image trials and 15 of 16 spoken-caption trials. The zero,
random-coordinate and unrelated-concept controls produced the target answer
once in 144 trials.

A second, disjoint confirmation held the intervention fixed and changed only
the question. The same exchange produced `Mammalia` for **biological class**
in 10 of 18 image trials and 10 of 18 spoken-caption trials, against 0 of 90
control trials per route. Text carried this second property weakly (3 of 18)
and is reported as a secondary outcome, frozen before the confirmation set was
opened.

The claim this supports is narrow: in this model, for this concept pair and
this direction, one exchanged identity coordinate can drive more than one
downstream fact, and it does so whether the evidence arrived as a caption, a
photograph or a recording. The reverse direction does not work. See
[docs/endpoint_semantics.md](docs/endpoint_semantics.md) for what the scored
endpoint does and does not mean.
...
run metadata is preserved locally under
`runs/pilot_20260715T200437612150_311fd108c23a/` (not committed).
...
held-out activations); run metadata is kept locally under
`runs/jspace_20260716T170808536780_e4118850fb70/` (not committed), with the
derived analysis committed under
...
completed text results into the static browser application above. It predates
the causal and multimodal studies reported at the top of this README and does
not display them.
...
| `jlens/mmpilot/full_vocabulary.py` | The **unrestricted next-token endpoint**: original prompt, nothing appended, complete next-token distribution, global argmax, tie-aware vocabulary rank, single-token enforcement, and deterministic greedy generation as a secondary demonstration ([docs/endpoint_semantics.md](docs/endpoint_semantics.md)) |
| `jlens/mmpilot/endpoint_audit.py`, `scripts/write_endpoint_audit.py` | The repository-wide endpoint-semantics audit and the machine-readable claim ledger: every headline claim traced to the function that computes it, plus the blocking scan that refuses to let a restricted-candidate result be called "global top-1", "full-vocabulary", "the model's output" or "paper-comparable" (artifacts under [`reports/endpoint_audit/`](reports/endpoint_audit/)) |
| `jlens/mmpilot/full_vocab_study.py`, `jlens/mmpilot/endpoint_amend.py` | The corrected causal rerun on the completed populations, and the versioned amendments that relabel the completed reports without recomputing anything (`scientific_recompute = 0`) |
...
| `notebooks/` | Three research notebooks; superseded studies, their generator scripts and their execution tests were pruned at the tip and remain reachable in the git log |
| `notebooks/multimodal_jspace_matched_jlens_colab.ipynb` | The pooled multimodal J-lens study: the matched fit over balanced text, image and spoken-caption examples, and the alpha=1 exact bird-to-cat exchange across the contiguous L16-L40 band, scored on free generation |
| `notebooks/multimodal_jspace_workspace_replication_colab.ipynb` | The workspace replication and the all-layer scan |
| `notebooks/multimodal_jspace_coordinate_swap_mock_colab.ipynb` | MOCK-only coordinate-swap notebook: the algebra, the refusals, the controls and the involution on a synthetic world |
