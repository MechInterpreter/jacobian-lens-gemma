...
| `notebooks/multimodal_jspace_digit_reasoning_confirmation_colab.ipynb` | ACTIVE (12): separately fingerprinted v2 confirmation of the paper-style animal-identity coordinate exchange using Gemma's exact digit outputs `2`/`4`, unrestricted global top-1 plus one-token greedy parity, **alpha=1 exact exchange as the only nonzero intervention**, the validated L33-L40 band, a fresh population excluding the completed alpha=2 null, matched alpha=1 controls, exact-exchange diagnostics, and atomic per-forward-pass resume units |
