...
| `notebooks/multimodal_jspace_coordinate_swap_mock_colab.ipynb` | MOCK-only coordinate-swap notebook: the algebra, the refusals, the controls and the involution on a synthetic world. Its real-run switch raises — at the time it was written no admissible layer band existed, and the readout has to be an open prompt |
| `notebooks/multimodal_jspace_anthropic_band_swap_colab.ipynb` | ACTIVE (9): Anthropic's primary protocol — the exact two-coordinate exchange clamped at every original prompt position across **every physical layer of a contiguous band**. Fits and confirms the missing physical layers 33/34/36/37/39 at scale 250 first, because a band with a hole in it is refused ([docs/coordinate_swap_protocol.md](docs/coordinate_swap_protocol.md)) |
| `notebooks/multimodal_jspace_full_vocabulary_causal_validation_colab.ipynb` | ACTIVE (11): the endpoint correction and the final bounded rerun — the endpoint audit and its amendments (CPU), then the completed L33–L40 and three-modality populations rescored on the unrestricted next-token endpoint (GPU), then the verdicts from saved units alone (CPU) ([docs/endpoint_semantics.md](docs/endpoint_semantics.md)) |
| `notebooks/multimodal_jspace_digit_reasoning_confirmation_colab.ipynb` | ACTIVE (12): separately fingerprinted v3 confirmation of the paper-style animal-identity coordinate exchange using Gemma's exact digit outputs `2`/`4`, unrestricted global top-1 plus one-token greedy parity, **alpha=1 exact exchange as the only nonzero intervention**, the validated L33-L40 band, 64 fresh candidates per concept excluding both completed digit runs, the unchanged eight-example causal cells, matched alpha=1 controls, exact-exchange diagnostics, and atomic per-forward-pass resume units |
