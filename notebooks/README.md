...
| 8 | `multimodal_jspace_anthropic_reasoning_swap_colab.ipynb` | **Primary paper-method test (v2) and independent alpha=2 confirmation.** Uses Anthropic's published two-coordinate exchange once at each independently confirmed physical layer and compares hidden animal-coordinate swaps directly with matched leg-answer swaps in text, image and spoken audio. Its confirmation mode freezes bird->cat, excludes every v1/v2 photograph, requires 12 fresh capability-valid recordings per modality, labels alpha=2 as amplified, and smart-saves every unit. It also includes the CPU-only direction-matched amendment for the completed v2 report. | `mmpaper2_*` exploratory run, immutable direction-matched v3 amendment, or separately fingerprinted `mmpaperconfirm_*` run |
