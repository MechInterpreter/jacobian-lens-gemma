...
| 12 | `multimodal_jspace_digit_reasoning_confirmation_colab.ipynb` | **Exact-exchange confirmation (v2).** The completed alpha=2 digit run remains an immutable null showing that the amplified intervention was destructive. This separately fingerprinted study freezes the exact digit rows before opening a fresh image-disjoint population that also excludes the alpha=2 run, uses alpha=1 as the only nonzero strength, runs the L33-L40 two-coordinate exchange across text/image/spoken-caption inputs, verifies unrestricted global argmax with one-token greedy generation, and requires exact-exchange diagnostics, both directions, every modality, a direct-answer positive control, and matched alpha=1 controls. Every forward pass is an atomic resume unit; nothing is fitted. | `mmalpha1confirm_real_*` run and `alpha1_exact_swap_confirmation_report.json` |
