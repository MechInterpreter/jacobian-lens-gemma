...
| 8 | `multimodal_jspace_anthropic_reasoning_swap_colab.ipynb` | **Primary paper-method test (v2).** Uses Anthropic's published two-coordinate exchange once at each independently confirmed physical layer and compares hidden animal-coordinate swaps directly with matched leg-answer swaps in text, image and spoken audio. It screens a fresh synchronized population first, uses alpha=1 as primary, reports alpha=2 separately, keeps the final-token edit descriptive rather than blocking, and includes a CPU-only direction-matched amendment for the completed v2 report. | `mmpaper2_*` run, `paper_reasoning_swap_v2_report.json`, and an immutable direction-matched v3 amendment |
