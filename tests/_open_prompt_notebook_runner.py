...
"""Execute the open-prompt MOCK notebook's code cells in a clean interpreter.

Run as a subprocess by ``tests/test_open_prompt_notebook.py`` from a working
...
Usage::
    ...
    python tests/_open_prompt_notebook_runner.py <notebook.ipynb> [NAME=True ...]

Each ``NAME=True`` argument rewrites a committed ``NAME = False`` switch before
execution, the same edit a user makes by hand. The only switch this notebook has
is ``RUN_REAL_OPEN_PROMPT_STUDY``, and flipping it is expected to *raise*.
        ...
        json.dumps(
            {
                ...
                "overrides": overrides,
                "run_real_open_prompt_study": namespace["RUN_REAL_OPEN_PROMPT_STUDY"],
                "mode": namespace["MODE"],
